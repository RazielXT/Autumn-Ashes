...
void entityDestroyed(Ogre::Entity* e)
{
...
void loadActions(const XMLElement* rootElement, EventsManager* mEventMgr, OgreNewt::Body* body, bodyUserData* userD, OgreNewt::MaterialID* action_mat)
{
    ...
    {
        int i = 0;
        auto aID = SUtils::strtok_str(actionsStr, ';');
        while (!aID.empty() && i < 4)
        {
            //add action
            ...
            i++;
            aID = SUtils::strtok_str(actionsStr, ';');
        }
    ...
    if (!startupTasks.empty())
    {
        ...
        auto react = SUtils::strtok_str(startupTasks, separator);
        while (!react.empty())
        {
            ...
            react = SUtils::strtok_str(startupTasks, separator);
        }
    }
}
...
void loadGrassArea(const XMLElement* element, Entity* ent, SceneNode* node, Ogre::SceneManager *mSceneMgr)
{
    ...
    {
        String curMat = SUtils::strtok_str(materials, delim);
        Real curMinSizeW = Ogre::StringConverter::parseReal(SUtils::strtok_str(minSizesW, delim));
        Real curMinSizeH = Ogre::StringConverter::parseReal(SUtils::strtok_str(minSizesH, delim));
        Real curMaxSizeW = Ogre::StringConverter::parseReal(SUtils::strtok_str(maxSizesW, delim));
        Real curMaxSizeH = Ogre::StringConverter::parseReal(SUtils::strtok_str(maxSizesH, delim));

        Real curSwayDistr = Ogre::StringConverter::parseReal(SUtils::strtok_str(SwayDistributions, delim));
        Real curSwayLen = Ogre::StringConverter::parseReal(SUtils::strtok_str(SwayLengths, delim));
        Real curSwaySpeed = Ogre::StringConverter::parseReal(SUtils::strtok_str(SwaySpeeds, delim));
        Real curDensity = Ogre::StringConverter::parseReal(SUtils::strtok_str(densities, delim));
        ...
        if (usesDensityMap)
        {
            String curDenMat = SUtils::strtok_str(densityMaps, delim);
...
void loadGate(const XMLElement* element, Ogre::Entity* ent, SceneNode* node)
{
    ...
    auto id = getElementValue(element, "ID");
    auto timeout = getElementIntValue(element, "Timeout");
    auto minUnlock = getElementIntValue(element, "MinUnlock");

    if (type == "Field")
        Gate::createFieldGate(ent, id, timeout, minUnlock);
    if (type == "Vertical")
        Gate::createPhysicalGate(ent, Vector3(0,1,0), id, timeout, minUnlock);
    if (type == "Horizontal")
        Gate::createPhysicalGate(ent, node->getOrientation()*Vector3(1, 0, 0), id, timeout, minUnlock);
}

void loadGateLock(const XMLElement* element, Ogre::Entity* ent, SceneNode* node)
{
    auto type = getElementValue(element, "Type");
    auto id = getElementValue(element, "ID");
    auto timeout = getElementIntValue(element, "Timeout");

    GateLock::createLock(node, type, id, timeout);
}
...
void loadJoint(const XMLElement* element, SceneNode* node, Ogre::SceneManager *mSceneMgr)
{
    ...
    auto child = SUtils::strtok_str(children, ';');
    element = element->NextSiblingElement();
    auto parent = SUtils::strtok_str(parents, ';');
    ...
    while (!child.empty() && !parent.empty())
    {
        ...
        //next
        child = SUtils::strtok_str(children, ';');
        parent = SUtils::strtok_str(parents, ';');
...
void loadBridge(const XMLElement* element, SceneNode* node, Ogre::SceneManager *mSceneMgr, uint32 visibilityFlag)
{
    ...
    auto val = SUtils::strtok_str(posText, ',');
    target.x = Ogre::StringConverter::parseReal(val);
    val = SUtils::strtok_str(posText, ',');
...
TriggerInfo* loadTrigger(const XMLElement* element, bool nonphysical = false)
{
    ...
    {
        int i = 0;
        auto aID = SUtils::strtok_str(actions, ';');
        while (!aID.empty() && i < 4)
        {
            //add action
    ...
    if (userdataElement != NULL)
    {
        ...
        if (!document.Error())
        {
            ...
            if (root->Value() != NULL)
            {
                ...
                else if (rootTag == "Gate")
                {
                    loadGate(root, ent, node);
                }
                else if (rootTag == "GateLock")
                {
                    loadGateLock(root, ent, node);
                }
