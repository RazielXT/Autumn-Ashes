...
bool GuiSceneEdit::pressedKey(const OIS::KeyEvent &arg)
{
	...
	case OIS::KC_LEFT:
		...
		break;
	case OIS::KC_RIGHT:
		if (activeLvl == 1 && base[activeBaseId].type == EditRow::Save)
		{
			currentEdit->save();
		}
		else
		{
			if (activeLvl == 1 && base[activeBaseId].type == EditRow::Params)
				currentVars = currentEdit->getParams(activeVarId);
...
void GuiSceneEdit::updateBase()
{
	if (currentEdit)
	{
		...
		for (int i = 0; i < base.size(); i++)
		{
			...
			if (base[i].type == EditRow::Save)
				baseRowsCaption[i]->colour(Ogre::ColourValue(1, 0, 1, 1));
			else if (base[i].type == EditRow::Params)
				baseRowsCaption[i]->colour(Ogre::ColourValue(1, 1, 1, 1));
			else
				baseRowsCaption[i]->colour(Ogre::ColourValue(0.7f, 1, 0.3f, 1));
		}
	}
}
