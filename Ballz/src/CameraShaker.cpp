...
void CameraShaker::update(float time)
{
	...
	if (camShakeTimer >= camShakeTimerEnd)
	{
		...
		camShakeTimer = 0;
		camShakeTimerEnd = Ogre::Math::Clamp<float>((shakeSize*(0.5f + shakeFreq) * 50) / (timerVar + 1), 0.05f, shakeFreq);
		...
		else
		{
			...
			float camShakeDirectionZ = Ogre::Math::RangeRandom(0.1f, 0.4f)*shakeSize;
			...
			camShakePrev = camShakeTarget;
			camShakeTarget.FromAngleAxis(random*Ogre::Degree(std::min<float>(shakeSize*timerVar * 50, 65)), Ogre::Vector3(camShakeDirectionX, camShakeDirectionY, camShakeDirectionZ));
		}
	}
}
