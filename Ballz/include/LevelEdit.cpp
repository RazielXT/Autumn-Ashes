...
void LevelEdit::editChanged(EditVariable& var, const std::string& row)
{
	if (row == "PP")
	{
		mergeParams(ppVariables, { var }, true);
		applyPPEdit(var);
	}		
	if (row == "Scene")
	{
		mergeParams(envVariables, { var }, true);
		applySceneEdit(var);
	}		
}
...
void LevelEdit::init()
{
	...
	rows = { { level->name,EditRow::Caption } ,{ "Save",EditRow::Action },{ "PP",EditRow::Params },{ "Scene",EditRow::Params } };
...
void LevelEdit::applyPPEdit(EditVariable& var)
{
	...
	Global::gameMgr->updatePPSettings();
}
...
void LevelEdit::applySceneEdit(EditVariable& var)
{
	...
	if (var.name == "FogColor")
	{
		...
		Global::mSceneMgr->setFog(Ogre::FOG_LINEAR, level->ambientColor, 1, level->fogStartDistance, level->fogEndDistance);
	}	
